...
class User(object):
  ...
  def on_logined(self, pbr):
    if self._check_pb_response_ok(pbr):
      ...
      RLOG.info("%s_%s logined"%(self.name, self.id))
      self._make_gameresult()
  ...
  def on_arenaJoined(self, msg):
    ...
    RLOG.info("%s_%s arenaJoined %s"%(self.name, self.id, self.arena))
    self._sleep(random.randrange(1,settings.BEFORE_READY))
    self.make_ready()
    
  def on_arenaLeaved(self, msg):
    """我离开了一个房间"""
    RLOG.info("%s_%s arenaLeaved %s"%(self.name, self.id, self.arena))
    self.arena = None
    self._sleep(random.randrange(1,settings.BEFORE_LISTARENA))
    self.list_arena()
  ...
  def on_arenaStart(self, msg):
    """游戏开始"""
    RLOG.info("%s_%s arenaStarted %s"%(self.name, self.id, self.arena))
    self._sleep(settings.BEFORE_SUBRESULT)
    self.submit_result()
  
  def on_arenaEnded(self, msg):
    """游戏结束"""
    RLOG.info("%s_%s arenaEnded %s"%(self.name, self.id, self.arena))
