...
class Arena(User):
    ...
    def make_ready(self):
        ...
        if (self._check_pb_response_ok(r)):
            ...
            send = False
            if (r.code/1000 == 403):
                #the user is arena host and can not ready
                pass
            else:
...
#===============================================================================
    ...
    def on_arenaLeaved(self, msg):
        """我离开了一个房间"""
        if self.arena:  #平台BUG，强杀war3进程时向服务器提交了两次quit_game请求，后收到2次leave_arena消息然产生的双重动作
...
# Test VirtualMessage
#===============================================================================
class testVirtualMessage(Arena):
    def __init__(self, name, password):
        super(testVirtualMessage, self).__init__(name, password)
        settings.BEFORE_READY *= 100
        settings.BEFORE_LISTARENA *= 100
        settings.BEFORE_SUBRESULT *= 100
        settings.USER_RANDOM_SLEEP_TIME *= 100
    ...
    def on_arenaStarted(self, msg):
        """游戏开始"""
        RLOG.debug("%s_%s arenaStarted %s"%(self.name, self.id, self.arena))
        self._sleep(100 + settings.BEFORE_SUBRESULT)
        self.submit_result()